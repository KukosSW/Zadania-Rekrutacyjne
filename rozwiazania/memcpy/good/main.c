...
void* my_memcpy(void* const restrict dest, const void* const restrict src, const size_t bytes)
{
    ...
    const size_t chunks_64 = bytes / sizeof(uint64_t);
    const size_t reminder_bytes = bytes % sizeof(uint64_t);
    {

        uint64_t * restrict dest_ptr = dest;
        const uint64_t * restrict src_ptr = src;
